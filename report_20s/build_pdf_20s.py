...
MD_PATH = ROOT / "report_20s.md"
PDF_PATH = ROOT / "report_20s.pdf"
...
class PdfWriter:
    ...
    def heading(self, text: str, level: int) -> None:
        if level == 1:
            self.text(text, size=19, bold=True, gap=0.025, max_width=58)
        elif level == 2:
            self.text(text, size=15, bold=True, gap=0.018, max_width=66)
        else:
            self.text(text, size=12, bold=True, gap=0.014, max_width=76)
    ...
    def code_or_table(self, lines: list[str]) -> None:
        ...
        for line in lines:
            self.fig.text(0.08, self.y, line, fontproperties=FONT, fontsize=7.3, va="top", color="#1f2937")
            self.y -= line_height
        self.y -= 0.012
